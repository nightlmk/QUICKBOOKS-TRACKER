...
Automatically tracks the latest QuickBooks desktop installer versions (US Windows, 2022–2024) by checking Intuit's download servers every Monday.

## How it works

- A GitHub Actions workflow (`check-versions.yml`) runs `check_versions.py` every Monday at 6 AM UTC (or on demand via `workflow_dispatch`).
